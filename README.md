...
TutorSlot is a WordPress lesson-scheduling plugin for tutors, students and
...
TutorSlot falls back to WordPress `AUTH_KEY` when the dedicated constant is not
...
`tutorslot_test*` database and `ts_test_*` table prefix. See
...
- `src/Admin/` registers onboarding, TutorSlot screens and settings.
...
`tutorslot_help_video_url` filter; when it is empty, the page keeps the written
...
Use [GitHub issues](https://github.com/itsmanzur/tutorslot/issues) for
...
TutorSlot is licensed under GPL-2.0-or-later. Bundled third-party notices are in
