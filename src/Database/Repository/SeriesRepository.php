<?php
/**
 * Recurring maintenance-plan series rows.
 *
 * @package PlumberSlot
 */
...
final class SeriesRepository extends AbstractRepository {
	...
	/**
	 * @param array{technician_id:int, customer_id:int, rrule:string, total_count:int, interval_weeks?:int} $data Series fields.
	 */
	public function create( array $data ): int {
		$this->db->insert(
			...
			array(
				...
				'interval_weeks' => max( 1, (int) ( $data['interval_weeks'] ?? 1 ) ),
				'created_at'     => $this->now(),
			),
			array( '%d', '%d', '%s', '%d', '%d', '%s' )
		);
	...
	/**
	 * Active (non-cancelled) appointment count for progress labels like Visit 9/12.
	 */
