...
TutorSlot requires WordPress 6.4+ and PHP 8.1+. Install a complete release ZIP,
then activate **TutorSlot** under **Plugins**. A source checkout also needs
`composer install`, `npm ci` and `npm run build` before it is feature-complete.

Activation creates TutorSlot's database tables and the Tutor, Student and Parent
roles. The setup wizard opens once for a user who can manage TutorSlot.

For optional payment or meeting credentials, first add a stable encryption key
to `wp-config.php`:

```php
define( 'TUTORSLOT_ENCRYPTION_KEY', '<a long random secret>' );
```
...
TutorSlot stores times in UTC and displays them in the applicable tutor/student
timezone.

## 4. Configure subjects

Open **TutorSlot → Subjects**. Each subject belongs to one tutor and can define
...
For multi-tutor sites, administrators manage tutor accounts under **TutorSlot →
Tutors**. Tutors can manage their own availability, subjects and bookings but do
not receive site-wide settings access.

## 5. Publish booking

Use the **TutorSlot booking** block, or add a shortcode:

```text
[tutorslot tutor="tutor-slug"]
```

Optional shortcode attributes:

```text
[tutorslot tutor="tutor-slug" subject="123"]
```
...
TutorSlot registers Student, Parent and Tutor roles.
...
[tutorslot_dashboard view="tutor"]
```

## 7. Payments and lesson credits

Open **TutorSlot → Settings → Connections** to configure optional Stripe or
bKash credentials. Online payments remain unavailable until payments are
enabled and a gateway is fully configured.

- Stripe uses hosted Checkout; TutorSlot does not collect raw card details.
...
site-configured server-to-server OAuth application. TutorSlot creates meetings
after eligible bookings are confirmed and sends participants a signed,
time-limited TutorSlot join URL instead of the raw provider URL.
...
by TutorSlot itself; an add-on must handle the `tutorslot_send_sms` action.

## 9. Manage a booking

Open **TutorSlot → Bookings** to view lessons, reschedule or cancel, record
...
TutorSlot registers WordPress personal-data export and erasure callbacks and
...
Deactivation removes TutorSlot scheduled actions but preserves data. Uninstall
also preserves data unless **Delete all TutorSlot data when the plugin is
