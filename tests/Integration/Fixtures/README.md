...
`schema-v4.sql` is the frozen TutorSlot database-version 4 contract used as the
...
`tutorslot_payments` and `tutorslot_webhook_events`.

Use `PreviousSchemaFixture::install()` only inside the isolated destructive
integration-test database. The installer replaces current TutorSlot test tables
and sets `tutorslot_db_version` to `PreviousSchemaFixture::VERSION`.
...
The fresh-install smoke removes every TutorSlot table, option, transient, role, and
