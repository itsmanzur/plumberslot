...
final class Phase8MigrationTest extends \WP_UnitTestCase {
	...
	public function test_version_four_fixture_matches_the_pre_payment_schema_contract(): void {
		...
		$this->assertTrue( $this->column_exists( Schema::table( Schema::SUBJECTS ), 'status' ) );
		$this->assertTrue( $this->column_exists( Schema::table( Schema::LOCKS ), 'owner_id' ) );
		$this->assertTrue( $this->index_exists( Schema::table( Schema::BOOKINGS ), 'idx_tutor_start' ) );
		$this->assertFalse( $this->index_exists( Schema::table( Schema::BOOKINGS ), 'idx_tutor_end' ) );
	...
	public function test_version_four_migrates_to_the_current_schema(): void {
		...
		$this->assertTrue( $this->column_exists( Schema::table( Schema::LOCKS ), 'owner_id' ) );
		$this->assertTrue( $this->index_exists( Schema::table( Schema::BOOKINGS ), 'idx_tutor_start' ) );
		$this->assertTrue( $this->index_exists( Schema::table( Schema::BOOKINGS ), 'idx_tutor_end' ) );
		$this->assertFalse( $this->index_exists( Schema::table( Schema::BOOKINGS ), 'uq_slot' ) );
	}
	...
	public function test_fresh_activation_creates_the_current_empty_schema_and_defaults(): void {
		...
		$this->assertNotNull( get_role( Capabilities::ROLE_TUTOR ) );
		$this->assertNotNull( get_role( Capabilities::ROLE_STUDENT ) );
		$this->assertNotNull( get_role( Capabilities::ROLE_PARENT ) );
	...
	private function seed_version_four_data(): void {
		...
		$wpdb->insert(
			Schema::table( Schema::TUTORS ),
			array(
				'id'                => 41,
				'user_id'           => 501,
				'slug'              => 'migration-tutor',
				'display_name'      => 'Migration Tutor',
				'timezone'          => 'Asia/Dhaka',
				'bio'               => 'Preserve this tutor profile.',
		...
		$wpdb->insert(
			...
			array(
				...
				'notes'         => 'Keep formulas & parent instructions.',
				'created_at'    => $created,
				'updated_at'    => $updated,
			)
		);
	...
	private function persisted_data_snapshot(): array {
		...
		$snapshot = array();
		foreach ( array( Schema::TUTORS, Schema::SUBJECTS, Schema::CREDITS, Schema::BOOKINGS, Schema::LOCKS ) as $key ) {
			$table = Schema::table( $key );
